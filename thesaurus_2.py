...
for line in lines.split('\n'):
	...
	words=line.split(';')
	try:
		cur.execute('SELECT id FROM  WORDS WHERE word=\''+words[0]+'\'')
		wid=cur.fetchone()[0]
	except:
		wid=0
	print (str(wid)+'+'+words[0])
	for item in words[1:]:
		try:
			if item!="":
				cur.execute('SELECT id FROM  WORDS WHERE word=\''+item+'\'')
				cid=cur.fetchone()[0]
			else:
				cid=0
		except:
		...
		if cid!=0 and wid!=0:
			try:
				cur.execute('INSERT INTO thesaurus(word,synonim) VALUES (\''+str(wid)+'\',\''+str(cid)+'\')')
				con.commit()
			except:
				con.commit()
f.close()
