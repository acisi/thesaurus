...
for line in lines.split('\n'):
	word=line.split('|')[0]
	syn_list=''
	if word.count('(синоним)')==0 and word.count('(антоним)')==0 and word.count('(сходный термин)')==0 and word.count('(связанный термин)')==0:
		base_word=word.upper()
		cur.execute('SELECT id FROM  WORDS WHERE word=\''+base_word+'\'')
		wido=cur.fetchone()
		try:
			wid=wido[0]
		except:
			wid=0
	if word.count('(синоним)')!=0:
		syn_list=line.upper().split('|')[1:]
		for word in syn_list:
			...
			if sid!=0 and wid!=0:
				print (sid)
				try:
					cur.execute('INSERT INTO thesaurus(word,synonim) VALUES (\''+str(wid)+'\',\''+str(sid)+'\')')
					con.commit()
				except:
					con.commit()
	print (base_word+str(wid)+'>'+str(syn_list))
f.close()
cur.close()
con.close()
